...
function tree_element($entry, $key, $all_entries, $crud)
{
    if (! isset($entry->tree_element_shown)) {
        ...
        $label = object_get($entry, $crud->get('reorder.label'));
        $thumbnail = url("/").'/'.object_get($entry, $crud->get('reorder.thumbnail'));

        if(!empty($thumbnail)) $label = '<img width=70 src="'.$thumbnail.'">';

        // show the tree element
        echo '<li id="list_'.$entry->getKey().'">';
        echo '<div><span class="disclose"><span></span></span>'.$label.'</div>';
